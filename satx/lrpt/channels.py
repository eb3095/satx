"""LRPT channel model and config parsing."""
...
def parse_lrpt_channels(
    entries: Optional[Sequence[Mapping[str, Any]]],
) -> list[SatChannel]:
    source = DEFAULT_LRPT_CHANNELS if not entries else entries
    return [channel_from_dict(item) for item in source]


DEFAULT_LRPT_CHANNEL_LIST = parse_lrpt_channels(None)
